...
def add_info(comic_info: ComicInfo, show_variants: bool = False) -> ComicInfo:
    if "league of comic geeks" in [x.website.lower() for x in comic_info.identifiers]:
        ...
        comic_id = search_comic(
            series_title=comic_info.series.title,
...
def parse_comic_result(result: Dict[str, Any], comic_info: ComicInfo) -> ComicInfo:
    ...
    if "league of comic geeks" not in [x.website.lower() for x in comic_info.series.publisher.identifiers]:
        comic_info.series.publisher.identifiers.append(
            IdentifierInfo(website="League of Comic Geeks", identifier=result["series"]["publisher_id"])
        )
    ...
    if "league of comic geeks" not in [x.website.lower() for x in comic_info.series.identifiers]:
        comic_info.series.identifiers.append(
            IdentifierInfo(website="League of Comic Geeks", identifier=result["series"]["id"])
        )
    ...
    if "league of comic geeks" not in [x.website.lower() for x in comic_info.identifiers]:
        comic_info.identifiers.append(
            IdentifierInfo(website="League of Comic Geeks", identifier=result["details"]["id"])
        )
    # TODO: Number
    # TODO: Title
...
def search_comic(
    series_title: str, comic_format: str, number: Optional[str] = None, show_variants: bool = False
) -> Optional[int]:
...
def __request(endpoint: str, params: List[Tuple[str, str]] = None) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    ...
    try:
        response = get(
            url=f"https://leagueofcomicgeeks.com/api{endpoint}",
            headers={"X-API-KEY": LOCG_API_KEY, "X-API-CLIENT": LOCG_CLIENT_ID},
            params=params,
        )
