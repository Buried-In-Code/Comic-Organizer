...
def add_info(comic_info: ComicInfo) -> ComicInfo:
    if "comicvine" in [x.website.lower() for x in comic_info.series.publisher.identifiers]:
        publisher_id = [
            x.identifier for x in comic_info.series.publisher.identifiers if x.website.lower() == "comicvine"
        ][0]
    else:
    ...
    if "comicvine" in [x.website.lower() for x in comic_info.identifiers]:
        ...
        issue_id = search_issues(
            volume=[x.identifier for x in comic_info.series.identifiers if x.website.lower() == "comicvine"][0],
...
def parse_publisher_result(result: Dict[str, Any], comic_info: ComicInfo) -> ComicInfo:
    ...
    if "comicvine" not in [x.website.lower() for x in comic_info.series.publisher.identifiers]:
        comic_info.series.publisher.identifiers.append(
            IdentifierInfo(website="Comicvine", identifier=result["id"], url=result["site_detail_url"])
        )
...
def parse_volume_result(result: Dict[str, Any], comic_info: ComicInfo) -> ComicInfo:
    ...
    if "comicvine" not in [x.website.lower() for x in comic_info.series.identifiers]:
        comic_info.series.identifiers.append(
            IdentifierInfo(website="Comicvine", identifier=result["id"], url=result["site_detail_url"])
        )
...
def search_publishers(name: str) -> Optional[int]:
    ...
    if len(results) >= 1:
        ...
        index = Console.display_menu(
            items=[f"{item['id']} | {item['name']} - {item['site_detail_url']}" for item in results],
            exit_text="None of the Above",
            prompt="Select Publisher",
        )
        if 1 <= index <= len(results):
...
def search_volumes(publisher: int, name: str, start_year: Optional[int] = None) -> Optional[int]:
    ...
    offset = 0
    params = [
        ("filter", f"publisher:{publisher},name:{name}"),
        ("field_list", "id,name,site_detail_url,publisher,start_year"),
    ]
    ...
    if len(results) >= 1:
        results = sorted(
            results, key=lambda x: (x["name"] is None, x["name"], x["start_year"] is None, x["start_year"])
        )
        index = Console.display_menu(
            items=[
                f"{item['id']} | {item['name']} [{item['start_year']}] - {item['site_detail_url']}" for item in results
            ],
            exit_text="None of the Above",
            prompt="Select Volume",
        )
...
def search_issues(volume: int, issue_number: str) -> Optional[int]:
    ...
    offset = 0
    params = [
        ("filter", f"volume:{volume},issue_number:{issue_number}"),
        ("field_list", "id,name,site_detail_url,volume,issue_number"),
    ]
    ...
    if len(results) >= 1:
        results = sorted(
            results, key=lambda x: (x["issue_number"] is None, x["issue_number"], x["name"] is None, x["name"])
        )
        index = Console.display_menu(
            items=[
                f"{item['id']} - #{item['issue_number']} {item['name']} - {item['site_detail_url']}" for item in results
            ],
            exit_text="None of the Above",
            prompt="Select Issue",
        )
...
def select_issue(issue_id: int) -> Dict[str, Any]:
    params = [
        (
            "field_list",
            "id,name,api_detail_url,issue_number,deck,image,site_detail_url,volume,cover_date,person_credits",
        )
    ]
...
def __request(endpoint: str, params: List[Tuple[str, str]] = None) -> Dict[str, Any]:
    ...
    try:
        response = get(
            ...
            headers={"User-Agent": "Comic-Organizer", "Accept": "application/json", "Content-Type": "application/json"},
            params=temp,
        )
