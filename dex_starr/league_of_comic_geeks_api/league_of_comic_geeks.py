...
class League:
    ...
    def lookup_comic(self, metadata: Metadata) -> Dict[str, Any]:
        if "league of comic geeks" in [x.service.lower() for x in metadata.comic.identifiers]:
            comic = self.select_comic(
                int([x for x in metadata.comic.identifiers if x.service.lower() == "league of comic geeks"][0].id)
            )
            if comic:
                return {
                    ...
                    "series": {
                        ...
                        "volume": {
                            "League of Comic Geeks": int(comic["series"]["volume"]) if comic["series"]["volume"] else 1
                        },
                        "start_year": {
                    ...
                    "comic": {
                        "format": {"League of Comic Geeks": FormatEnum.get(comic["details"]["format"])},
                        "cover_date": {"League of Comic Geeks": date.fromisoformat(comic["details"]["date_release"])},
                        "page_count": {"League of Comic Geeks": int(comic["details"]["pages"])},
                        "summary": {"League of Comic Geeks": self.__remove_extra(comic["details"]["description"])},
                    },
                }
        else:
            ...
            if result:
                metadata.publisher.identifiers.append(
                    Identifier(service="League of Comic Geeks", id=int(result["publisher_id"]))
                )
                metadata.series.identifiers.append(
                    Identifier(service="League of Comic Geeks", id=int(result["series_id"]))
                )
                metadata.comic.identifiers.append(Identifier(service="League of Comic Geeks", id=int(result["id"])))
    ...
    def search_comic(
        ...
        if results:
            ...
            if results:
                selected_index = CONSOLE.menu(
                    options=[f"{x['id']} | {x['title']}" for x in results], prompt="Select Comic"
                )
                if selected_index != 0:
        ...
        if attempts:
            CONSOLE.info("No results found in League of Comic Geeks")
        search = CONSOLE.prompt("Search Term", require_response=False)
        if search:
...
def pull_info(api_key: str, client_id: str, cache: SQLiteCache, metadata: Metadata):
    ...
    CONSOLE.rule("Pulling Info from League of Comic Geeks")
